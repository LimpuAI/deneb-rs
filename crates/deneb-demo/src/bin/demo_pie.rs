...
use deneb_component::{
    PieChart, ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, Mark,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let csv = sample_data::pie_chart_csv();
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| PieChart::render(&spec, t, &table), "Deneb - Pie Chart"),
        Some("forest") => render_and_show(ForestTheme, |t| PieChart::render(&spec, t, &table), "Deneb - Pie Chart"),
        Some("nordic") => render_and_show(NordicTheme, |t| PieChart::render(&spec, t, &table), "Deneb - Pie Chart"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| PieChart::render(&spec, t, &table), "Deneb - Pie Chart"),
        _ => render_and_show(DefaultTheme, |t| PieChart::render(&spec, t, &table), "Deneb - Pie Chart"),
    }
}
...
fn run_wasm(host: &mut WasmHost, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let wit_spec = WitChartSpec {
        ...
        width: 800.0,
        height: 600.0,
        title: Some("Pie Chart Demo (WASM)".to_string()),
    ...
    let app = DemoApp::new("Deneb - Pie Chart (WASM)", 800, 600);
