...
use deneb_component::{
    StripChart, ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, Mark,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let csv = sample_data::strip_chart_csv();
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| StripChart::render(&spec, t, &table), "Deneb - Strip"),
        Some("forest") => render_and_show(ForestTheme, |t| StripChart::render(&spec, t, &table), "Deneb - Strip"),
        Some("nordic") => render_and_show(NordicTheme, |t| StripChart::render(&spec, t, &table), "Deneb - Strip"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| StripChart::render(&spec, t, &table), "Deneb - Strip"),
        _ => render_and_show(DefaultTheme, |t| StripChart::render(&spec, t, &table), "Deneb - Strip"),
    }
}

fn run_wasm(host: &mut WasmHost, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let wit_spec = WitChartSpec {
        mark: "strip".to_string(),
        ...
        width: 800.0,
        height: 600.0,
        title: Some("Strip Chart Demo (WASM)".to_string()),
    ...
    let app = DemoApp::new("Deneb - Strip (WASM)", 800, 600);
