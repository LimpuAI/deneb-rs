...
use deneb_component::{
    RadarChart, ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, Mark,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let csv = sample_data::radar_chart_csv();
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| RadarChart::render(&spec, t, &table), "Deneb - Radar"),
        Some("forest") => render_and_show(ForestTheme, |t| RadarChart::render(&spec, t, &table), "Deneb - Radar"),
        Some("nordic") => render_and_show(NordicTheme, |t| RadarChart::render(&spec, t, &table), "Deneb - Radar"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| RadarChart::render(&spec, t, &table), "Deneb - Radar"),
        _ => render_and_show(DefaultTheme, |t| RadarChart::render(&spec, t, &table), "Deneb - Radar"),
    }
}

fn run_wasm(host: &mut WasmHost, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let wit_spec = WitChartSpec {
        mark: "radar".to_string(),
        x_field: "dimension".to_string(),
        ...
        width: 800.0,
        height: 600.0,
        title: Some("Radar Chart Demo (WASM)".to_string()),
    ...
    let app = DemoApp::new("Deneb - Radar (WASM)", 800, 600);
