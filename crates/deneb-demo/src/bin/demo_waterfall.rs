...
use deneb_component::{
    WaterfallChart, ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, Mark,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let csv = sample_data::waterfall_chart_csv();
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| WaterfallChart::render(&spec, t, &table), "Deneb - Waterfall"),
        Some("forest") => render_and_show(ForestTheme, |t| WaterfallChart::render(&spec, t, &table), "Deneb - Waterfall"),
        Some("nordic") => render_and_show(NordicTheme, |t| WaterfallChart::render(&spec, t, &table), "Deneb - Waterfall"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| WaterfallChart::render(&spec, t, &table), "Deneb - Waterfall"),
        _ => render_and_show(DefaultTheme, |t| WaterfallChart::render(&spec, t, &table), "Deneb - Waterfall"),
    }
}

fn run_wasm(host: &mut WasmHost, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let wit_spec = WitChartSpec {
        mark: "waterfall".to_string(),
        ...
        width: 800.0,
        height: 600.0,
        title: Some("Waterfall Chart Demo (WASM)".to_string()),
    ...
    let app = DemoApp::new("Deneb - Waterfall (WASM)", 800, 600);
