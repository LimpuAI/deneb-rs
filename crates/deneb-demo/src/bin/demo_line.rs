...
use deneb_component::{
    ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, LineChart, Mark,
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| LineChart::render(&spec, t, &table), "Deneb - Line Chart"),
        Some("forest") => render_and_show(ForestTheme, |t| LineChart::render(&spec, t, &table), "Deneb - Line Chart"),
        Some("nordic") => render_and_show(NordicTheme, |t| LineChart::render(&spec, t, &table), "Deneb - Line Chart"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| LineChart::render(&spec, t, &table), "Deneb - Line Chart"),
        _ => render_and_show(DefaultTheme, |t| LineChart::render(&spec, t, &table), "Deneb - Line Chart"),
    }
}
