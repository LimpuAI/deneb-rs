...
use deneb_component::{
    HeatmapChart, ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, Mark,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let csv = sample_data::heatmap_chart_csv();
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| HeatmapChart::render(&spec, t, &table), "Deneb - Heatmap"),
        Some("forest") => render_and_show(ForestTheme, |t| HeatmapChart::render(&spec, t, &table), "Deneb - Heatmap"),
        Some("nordic") => render_and_show(NordicTheme, |t| HeatmapChart::render(&spec, t, &table), "Deneb - Heatmap"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| HeatmapChart::render(&spec, t, &table), "Deneb - Heatmap"),
        _ => render_and_show(DefaultTheme, |t| HeatmapChart::render(&spec, t, &table), "Deneb - Heatmap"),
    }
}

fn run_wasm(host: &mut WasmHost, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let wit_spec = WitChartSpec {
        mark: "heatmap".to_string(),
        x_field: "x".to_string(),
    ...
    let app = DemoApp::new("Deneb - Heatmap (WASM)", 800, 600);
