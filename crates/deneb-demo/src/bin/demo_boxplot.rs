...
use deneb_component::{
    BoxPlotChart, ChartSpec, DarkTheme, DefaultTheme, Encoding, Field, ForestTheme, Mark,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let csv = sample_data::boxplot_chart_csv();
...
fn run_direct(csv: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match theme_name.as_deref() {
        Some("dark") => render_and_show(DarkTheme, |t| BoxPlotChart::render(&spec, t, &table), "Deneb - Box Plot"),
        Some("forest") => render_and_show(ForestTheme, |t| BoxPlotChart::render(&spec, t, &table), "Deneb - Box Plot"),
        Some("nordic") => render_and_show(NordicTheme, |t| BoxPlotChart::render(&spec, t, &table), "Deneb - Box Plot"),
        Some("cappuccino") => render_and_show(CappuccinoTheme, |t| BoxPlotChart::render(&spec, t, &table), "Deneb - Box Plot"),
        _ => render_and_show(DefaultTheme, |t| BoxPlotChart::render(&spec, t, &table), "Deneb - Box Plot"),
    }
}
...
fn run_wasm(host: &mut WasmHost, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    let wit_spec = WitChartSpec {
        ...
        width: 800.0,
        height: 600.0,
        title: Some("Box Plot Demo (WASM)".to_string()),
    ...
    let app = DemoApp::new("Deneb - Box Plot (WASM)", 800, 600);
