...
///
/// Returns a `Vec<f64>` of the same length as `values`, where each value is
/// the offset in screen units along the spread axis.
...
fn beeswarm_greedy(values: &[f64], point_radius: f64, band_width: f64) -> Vec<f64> {
    ...
    // Placed points: (value, offset)
    // Circle-circle overlap test in (value, offset) space.
...
mod tests {
    ...
    fn test_beeswarm_no_overlap() {
        ...
        // Check that no two points overlap
    ...
    #[test]
    fn test_beeswarm_offsets_within_band() {
        let values: Vec<f64> = (0..20).map(|i| i as f64).collect();
        let band_width = 60.0;
        let offsets = beeswarm_layout(&values, StripLayout::Beeswarm, 3.0, band_width);
        for o in &offsets {
            assert!(
                o.abs() <= band_width * 0.45 + 0.01,
                "offset {o} exceeds max band width"
            );
        }
    }
    ...
    fn test_empty_values() {
        let offsets = beeswarm_layout(&[], StripLayout::Beeswarm, 4.0, 40.0);
        assert!(offsets.is_empty());
    }

    #[test]
    fn test_single_value() {
        let offsets = beeswarm_layout(&[42.0], StripLayout::Beeswarm, 4.0, 40.0);
