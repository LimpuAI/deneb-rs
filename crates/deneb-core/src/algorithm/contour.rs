...
/// A single iso-level contour path
#[derive(Debug, Clone, PartialEq)]
pub struct ContourPath {
    /// The iso-value for this contour
    pub level: f64,
    /// Connected polyline points in screen coordinates.
    /// Each inner `Vec<(f64, f64)>` is one connected polyline segment.
...
pub fn marching_squares(
    ...
    levels
        ...
        .map(|&level| {
            ...
            // Chain-stitch adjacent segments into longer polylines
            let paths = stitch_segments(paths);

            ContourPath { level, paths }
        })
        .collect()
}
...
///
/// When a contour arc exits the grid, auto-close draws a diagonal line
...
mod tests {
    ...
    fn test_multiple_levels() {
        ...
        let result = marching_squares(&grid, &[0.5, 1.5], (0.0, 10.0), (0.0, 10.0));
        assert_eq!(result.len(), 2);
        // Each level should produce at least one path
        assert!(!result[0].paths.is_empty());
        assert!(!result[1].paths.is_empty());
    }

    #[test]
    fn test_empty_grid() {
        let result = marching_squares(&[], &[0.5], (0.0, 10.0), (0.0, 10.0));
        assert_eq!(result.len(), 1);
        assert!(result[0].paths.is_empty());
    }
